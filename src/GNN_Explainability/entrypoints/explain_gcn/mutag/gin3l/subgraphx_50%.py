...
class Entrypoint(SubgraphXEntrypoint):
    def __init__(self):
        conf = SubgraphXConfig(
            ...
            edge_mask_save_dir=os.path.join('..', 'data', 'ba_2motifs', 'explanation', 'gin3l', 'subgraphx_50%'),
            sparsity=0.0,
        ...
        model.load_state_dict(torch.load('../results/12_gin3l_MUTAG/106', map_location=conf.device))
