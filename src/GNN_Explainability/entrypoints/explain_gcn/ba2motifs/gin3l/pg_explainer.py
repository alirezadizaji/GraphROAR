...
class Entrypoint(MainEntrypoint):
    ...
    def run(self):
        ...
        explainer = PGExplainer(self.model, in_channels=600, 
        ...
        torch.save(explainer.state_dict(), '../results/4_pgexplainer_gin3l_BA2Motifs/weights/model.pt')
        # explainer.load_state_dict(torch.load('../results/4_pgexplainer_gin3l_BA2Motifs/weights/model.pt', map_location=self.conf.device))
        ...
        for loader in [self.train_loader, self.val_loader, self.test_loader]:
            for i, data in enumerate(loader):
                ...
                if i_total <= 20:
                    ...
                    save_dir='../results/4_pgexplainer_gin3l_BA2Motifs/visualizations')
                    plt.close()
                ...
                file_dir = os.path.join('..', 'data', 'ba_2motifs', 'explanation', 'pgexplainer')
